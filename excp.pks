...
bcoulam      2012Jan24 Removed second version of throw(). Not needed without
                       re-usable messages scheme.
...
/**-----------------------------------------------------------------------------
throw:
 Throw is mainly used internally, by the logs.msg routine for its exception handling.
 Replacing direct calls to RAISE_APPLICATION_ERROR helps reduce hard-code, 
 enforcing consistent exception handling.
 
 In general, whenever you trap an error in a calling routine, you will always 
 call logs.err to log the runtime context. logs.err, by default, will then re-raise 
 the Oracle or user-defined exception by calling throw().
 
%design
 I chose "throw" because "raise" is a reserved word in PL/SQL. Our Oracle tools
 uppercase "raise" when typed. This meant that you'd have to go back and change
 it back to lowercase to meet format standards. Not fun. So I went with "throw" 
 ...
 an error that warrants attention, but not immediate, you will call logs.warn.
 If it is or could be disastrous, call logs.err, e.g.

    logs.err('Incoming Queue Error: Payload is full.');

 which logs the message and then calls excp.throw to re-raise the exception.
...
%param i_msg_id Positive or negative error number, from list of Oracle error 
                numbers (allowed user-defined range, or Oracle pre-defined).
