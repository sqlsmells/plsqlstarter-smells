...
bcoulam      2012Jan24 Changed from dynamic app_parm_vw to simple table lookup
                       based on app_id.
<i>
...
FUNCTION get_val(i_parm_nm IN app_parm.parm_nm%TYPE)
   RETURN app_parm.parm_val%TYPE
IS
   l_val app_parm.parm_val%TYPE;
BEGIN
   SELECT parm_val
     INTO l_val
     FROM app_parm
...
EXCEPTION
   WHEN NO_DATA_FOUND THEN
      RAISE_APPLICATION_ERROR(-20000,
         'Parameter "'||i_parm_nm||'" is not found in APP_PARM.'); 
END get_val;
