...
BEGIN
   IF (io_prob_id IS NULL) THEN
      SELECT ps_prob_seq.nextval INTO io_prob_id FROM dual;
   END IF;
...
BEGIN
   SELECT ps_sol_seq.nextval INTO lr_sol.sol_id FROM dual;
...
BEGIN
   ...
   excp.assert(i_sol_id IS NOT NULL, 'Cannot delete a solution without a solution ID', TRUE);
