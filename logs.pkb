...
,  i_call_stack IN VARCHAR2 DEFAULT NULL
)
  RETURN VARCHAR2
IS
...
BEGIN
   RETURN SUBSTR(
      ...
      NVL(i_msg,'[No Message Given]')||
      CASE WHEN i_error_stack IS NOT NULL THEN cnst.PIPECHAR||CHR(10)||i_error_stack ELSE NULL END||
      CASE WHEN i_call_stack IS NOT NULL THEN cnst.PIPECHAR||CHR(10)||i_call_stack ELSE NULL END
...
BEGIN
   RETURN SUBSTR(NVL(i_msg, '[No Message Given]'), 1, cnst.MAX_COL_LEN);
END format_tbl_txt;
...
------------------------------------------------------------------------------*/
PROCEDURE vld_debug_toggle_format(i_parm_val IN app_parm.parm_val%TYPE)
IS
   l_parm_val app_parm.parm_val%TYPE := LOWER(TRIM(i_parm_val));
BEGIN
...
/**-----------------------------------------------------------------------------
to_file:
 The to_file routine is only used by the logs.msg routine if File is one of the 
 log targets.
...
%param i_line_num Optional line number from where the message originated
%param i_error_stack Optional call hierarchy which pinpoints the line from which the
                     error originated.
%param i_call_stack Optional call hierarchy.
...
BEGIN
   io.write_line(format_log_txt(i_msg, i_sev_cd, i_routine_nm, i_line_num, i_error_stack, i_call_stack),
...
/**-----------------------------------------------------------------------------
to_table:
 The to_table routine is only used by the logs.msg routine if Table is one of 
 the log targets.

 When to_table is called the message will be inserted into the
 app_log table.
...
PROCEDURE to_table
(
   i_msg         IN app_log.log_txt%TYPE, -- must be <= 4K
   i_sev_cd      IN app_log.sev_cd%TYPE DEFAULT cnst.INFO,
   i_routine_nm  IN app_log.routine_nm%TYPE DEFAULT NULL,
   i_line_num    IN app_log.line_num%TYPE DEFAULT NULL,
   i_error_stack IN app_log.error_stack%TYPE DEFAULT NULL,
...
/**-----------------------------------------------------------------------------
msg:
 Private routine. Called by info, warn, err and dbg.
 
 msg() can re-raise an exception if it is explicitly asked to.
 If you just want the message to be logged, leave the i_reraise parameter blank.
 If you want to halt processing, set i_reraise to TRUE. For example, wrap your 
 ...
 Ensure you let msg() know what type of message you want logged (%see cnst.DEBUG,
 cnst.INFO, cnst.WARN and cnst.ERROR). Will default to INFO if not given.
 Remember to explicitly pass in ERROR or WARN for the severity if you wish the
 messages to be seen by the log-scanner. The log-scanning app does not come with
 the framework. It must be custom-built per each shop's needs. It is usually 
 implemented as a cron or scheduled Oracle job that reads through the latest N 
 minutes of APP_LOG records, recording the last-scanned timestamp somewhere. It
 then emails certain people or groups with high-severity errors so that issues 
 in production can be proactively detected and sometimes handled before the end 
 users even notice or report them.

%param i_msg Application message. Will be truncated if longer than 32K.
%param i_err_id Optional error ID. Typically the Oracle Error Message ID (SQLCODE).
                In the full Starter framework, this parameter is i_msg_id and
                can automatically tie into positive or negative messages in
                APP_MSG.
...
(
   ...
   i_err_id      IN INTEGER DEFAULT NULL,
   i_sev_cd      IN app_log.sev_cd%TYPE DEFAULT cnst.INFO,
...
BEGIN
   ...
   IF (g_to_screen) THEN
      io.p(format_log_txt(i_msg, NVL(i_sev_cd, cnst.INFO), l_routine_nm, l_line_num, i_error_stack, i_call_stack));
   END IF;
   
   IF (g_to_table) THEN
      to_table(i_msg, NVL(i_sev_cd, cnst.INFO), l_routine_nm, l_line_num, i_error_stack, i_call_stack);
   END IF;
   
   IF (g_to_file) THEN
      to_file(i_msg, NVL(i_sev_cd, cnst.INFO), l_routine_nm, l_line_num, i_error_stack, i_call_stack);
   END IF;
   
   -- Now that logging is complete, re-raise exception if caller wants it.
   IF (i_reraise) THEN
      excp.throw(i_err_id, i_msg);
   END IF;
...
BEGIN
   RETURN
   TARGET_SCREEN||'['||str.bool_to_str(g_to_screen)||'] '||
   TARGET_TABLE||'['||str.bool_to_str(g_to_table)||'] '||
   TARGET_FILE||'['||str.bool_to_str(g_to_file)||']';
END get_targets;
...
IS
   l_msg         typ.t_maxvc2;
   l_error_stack app_log.error_stack%TYPE;
   l_call_stack  app_log.call_stack%TYPE;
   l_routine_nm  app_log.routine_nm%TYPE := i_routine_nm;
   l_line_num    app_log.line_num%TYPE := i_line_num;
BEGIN
   IF (i_routine_nm IS NULL) THEN
   ...
   -- ensure logged message isn't blank
   IF (i_msg IS NULL AND SQLCODE <> 0) THEN
      l_msg := SQLERRM;
   ELSIF (i_msg IS NULL AND SQLCODE = 0) THEN
      l_msg := '[Message Not Given]'; -- empty message for a useless, user-defined error
   ELSE
      l_msg := i_msg;
   END IF;
   ...
   IF (i_reraise OR SQLCODE <> 0) THEN
      IF (SQLCODE = 0) THEN
   ...
   msg(l_msg, SQLCODE, cnst.ERROR, i_reraise, l_routine_nm, l_line_num, l_error_stack, l_call_stack);
END err;
...
BEGIN
   ...
   msg(i_msg, NULL, cnst.WARN, FALSE, l_routine_nm, l_line_num);
END warn;
...
BEGIN
   ...
   msg(i_msg, NULL, cnst.INFO, FALSE, l_routine_nm, l_line_num);
END info;
...
BEGIN
   ...
   IF (gr_debug.debugging_on OR gr_debug.session_override) THEN
      ...
      IF ( (gr_debug.debug_type = 'all') OR -- this one will be used 99% of the time,
         ...
         to_table(i_msg, cnst.DEBUG, l_routine_nm, l_line_num);
         
         -- also goes to stdout if it is a desired target
         IF (g_to_screen) THEN
            io.p(format_log_txt(i_msg, cnst.DEBUG, l_routine_nm, l_line_num));
         END IF;
         
         -- also goes to file if it is a desired target
         IF (g_to_file) THEN
            to_file(i_msg, cnst.DEBUG, l_routine_nm, l_line_num);
         END IF;
      ELSE
...
IS
   ...
   BEGIN
      RETURN SUBSTR(TO_CHAR(ir_app_log.log_ts, 'YYYY/MM/DD') || cnst.PIPECHAR ||
                    ...
                    CASE WHEN ir_app_log.error_stack IS NOT NULL THEN cnst.PIPECHAR ||'ERROR STACK ['||ir_app_log.error_stack||']' ELSE NULL END||
                    CASE WHEN ir_app_log.call_stack IS NOT NULL THEN cnst.PIPECHAR ||'CALL STACK ['||ir_app_log.call_stack||']' ELSE NULL END,
                    1,
...
BEGIN
   ...
   g_file_nm := parm.get_val('Default IO File Name');
