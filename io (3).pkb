...
/**-----------------------------------------------------------------------------
 ...
 Private iget_my_nmmplemenation of the same function in ENV so that IO, which is a low-level
 package, does not have a dependency on the higher-level packag ENV.
------------------------------------------------------------------------------*/
FUNCTION get_my_nm RETURN VARCHAR2
IS
BEGIN
      ...
      IF (file_exists(i_new_file_nm, i_old_file_dir)) THEN
         ...
         ELSE
            ...
            app_log_api.ins('File Rename Request. Location ['||i_old_file_dir||'] '||
               'OLD['||i_old_file_nm||'] '||
               'NEW['||i_new_file_nm||']',
               cnst.INFO, msgs.DEFAULT_MSG_CD, get_my_nm);
         END IF;
      ELSE
         utl_file.frename(i_old_file_dir,
                          i_old_file_nm,
